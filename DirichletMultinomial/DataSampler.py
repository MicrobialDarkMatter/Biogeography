...
from DirichletMultinomial.misc.distance_matrix import get_distance_matrix  # TODO: Fix import when moved
...
class DataSampler(Dataset):
    def __init__(self, Y_path, X_path, coords_path, device, normalize_X: bool, traits_path: str="", prevalence_threshold=0, taxonomy_level="", total_counts_path=""):
        ...
        if self.taxonomy_level:  # Taxonomy aggregation
            ...
            taxonomy = taxonomy.set_index("Taxon").loc[self.Y_cols_species].reset_index()
            taxonomy = taxonomy[cols_taxonomy]
            groups = taxonomy.groupby(self.taxonomy_level).groups  # Do aggregation

            self.Y = torch.stack([self.Y[:, v].sum(dim=1) for v in groups.values()]).T
            self.Y = self.Y / self.Y.sum(dim=1).unsqueeze(1)
            self.Y_cols_species = list(groups.keys())

            # Removing zero columns and rows
            col_mask = self.Y.abs().sum(dim=0) != 0  # keep columns that have any nonzero
            self.Y = self.Y[:, col_mask]

            row_mask = self.Y.abs().sum(dim=1) != 0  # keep rows that have any nonzero
            self.Y = self.Y[row_mask]

            print(f"Hierarchy: {self.taxonomy_level} // Unique: {len(groups.keys())} // Kept: {self.Y.shape[1]}")

        if True:  # Transform relative percentages to counts
            if total_counts_path:
                ...
                factor = 1e6
                print(f"Multiplying relative percentages with {factor} to get counts")

                self.Y = (self.Y * factor).round().int()
