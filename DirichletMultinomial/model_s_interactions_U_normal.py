...
from DirichletMultinomial.MultitaskVariationalStrategy import MultitaskVariationalStrategy
...
class MicroGP(pyro.nn.PyroModule):
    ...
    def model(self, batch):
        ...
        # Species correlation
        # # Concentration parameter (>1 favors identity/correlated less)
        # # Only model (+, +) or (-, -) not (+, -) or (-, +)
        # L_corr = pyro.sample("L_corr", dist.LKJCholesky(dim=n_species, concentration=100)).squeeze()
        # U = (L_corr @ L_corr.T)

        with pyro.plate(name="species_plate1", size=n_species, dim=-2), species_plate:
            U = pyro.sample("U", dist.Normal(loc=torch.zeros(n_species, n_species), scale=torch.ones(n_species, n_species) * 0.1))
        ...
        #     pyro.sample("y", dist.Dirichlet(z), obs=batch.get("Y") if batch.get("training", True) else None)

        # example skeleton with explicit dims

        if batch.get("training", True):
            with samples_plate:
                # IMPORTANT: no species_plate here; species is the Dirichlet EVENT dim
                pyro.sample("y", dist.DirichletMultinomial(concentration=z, total_count=batch.get("Y").sum(dim=1)), obs=batch.get("Y"))
        else:
            # No plate for predictive !!!
            pyro.sample("y", dist.DirichletMultinomial(concentration=z, total_count=int(1e5)), obs=None)
    ...
    def guide(self, batch):
        ...
        # eta_param = pyro.param("correlation_eta", torch.tensor(100.0), constraint=dist.constraints.positive)
        #
        # # Sample L_corr from LKJCholesky prior
        # pyro.sample("L_corr", dist.LKJCholesky(dim=n_species, concentration=eta_param))

        U_loc = pyro.param(
            "U_loc",
            torch.zeros(n_species, n_species)
        )
        U_scale = pyro.param(
            "U_scale",
            0.1 * torch.ones(n_species, n_species),
            constraint=dist.constraints.positive
        )

        with pyro.plate("species_plate1", size=n_species, dim=-2), species_plate:
            pyro.sample("U", dist.Normal(U_loc, U_scale))
...
if __name__ == "__main__":
    ...
    from DirichletMultinomial.DataSampler import DataSampler

    from DirichletMultinomial.misc.calculate_metrics import calculate_metrics, precision_at_k
    ...
    #     y_prob = y_prob / y_prob.sum(dim=1).unsqueeze(1) * dataset.total_counts[idx]
    ...
    #
    # # TODO: predictive does not support differing total_counts
    #
    ...
    for idx in test_dataloader:
        ...
        z = predictive(batch)["z"].mean(dim=0).squeeze()

        prob_list.append(dist.Dirichlet(z).mean)
